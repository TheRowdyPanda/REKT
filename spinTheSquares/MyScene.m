...
@property (nonatomic) NSTimeInterval timeSinceLastCar;
@property (nonatomic) NSTimeInterval timeSinceLastBag;
  ...
  //  [self updateScoreLabel:0];
    ...
    */
    ...
    if(self.size.height == 480){
        NSMutableArray *instructFrames = [NSMutableArray array];
        SKTextureAtlas *instructAtlas = [SKTextureAtlas atlasNamed:@"instructions_66"];
        
        
        int numImages = instructAtlas.textureNames.count;
        for (int i=1; i <= numImages; i++) {
            NSString *textureName = [NSString stringWithFormat:@"instructions_66_%d", i];
            SKTexture *temp = [instructAtlas textureNamed:textureName];
            [instructFrames addObject:temp];
        }
        
        
        // _crappyBird = [SKSpriteNode spriteNodeWithImageNamed:@"bird1"];
        ...
        instructions.name = @"instructionsNode";
        [self addChild:instructions];
        //[instructions runAction:[SKAction animateWithTextures:instructFrames timePerFrame:0.1]];
        [instructions runAction:[SKAction repeatAction:[SKAction animateWithTextures:instructFrames timePerFrame:0.1] count:-1]];
    }
    else{
    NSMutableArray *instructFrames = [NSMutableArray array];
    SKTextureAtlas *instructAtlas = [SKTextureAtlas atlasNamed:@"instructions"];
    
    
    int numImages = instructAtlas.textureNames.count;
    for (int i=1; i <= numImages; i++) {
        NSString *textureName = [NSString stringWithFormat:@"instructions_75_%d", i];
        SKTexture *temp = [instructAtlas textureNamed:textureName];
        [instructFrames addObject:temp];
    }
    ...
    [tracker set:kGAIScreenName value:@"Main Gamxeplay View Started"];
...
-(void)addCar{
    ...
    if(_gameIsOver == false && _gameHasStarted == TRUE){
        ...
        for (int i = 0 ; i < posSize; i++) {
            ...
            int checkNum = (8 - floor(_score/(50.0*pointMult))); //max at 400
            if(checkNum <=1) checkNum = 1;
            //checkNum = 1;
            int chanceToAddSquare = arc4random()%checkNum;
            if(chanceToAddSquare == 0 || chanceToAddSquare == 1){
        
        

                SKSpriteNode * car1Node = [SKSpriteNode spriteNodeWithColor:[UIColor whiteColor] size:CGSizeMake(40*_sizeChanger, 40*_sizeChanger)];
...
-(void)setColor{
    ...
    if(num == 31){
        ...
        _truckColor = [self colorWithHexString:@"A4B1C2"];
        _bagColor = [self colorWithHexString:@"6A7B8D"];
        _obstacleColor = [self colorWithHexString:@"2B3F5A"];
        _backColor = [self colorWithHexString:@"162A42"];
        
    }
    if(num == 32){
        
        _truckColor = [self colorWithHexString:@"DF5C5F"];
        _bagColor = [self colorWithHexString:@"4B505C"];
        _obstacleColor = [self colorWithHexString:@"F4F9FC"];
        _backColor = [self colorWithHexString:@"A1BAC1"];
        
    }
    if(num == 33){
        
        _truckColor = [self colorWithHexString:@"E8FFA2"];
        _bagColor = [self colorWithHexString:@"ABE872"];
        _obstacleColor = [self colorWithHexString:@"A3FF8A"];
        _backColor = [self colorWithHexString:@"63E86A"];
        
    }
    if(num == 34){
        
        _truckColor = [self colorWithHexString:@"7542FF"];
        _bagColor = [self colorWithHexString:@"4CDEE8"];
        _obstacleColor = [self colorWithHexString:@"8AFF60"];
        _backColor = [self colorWithHexString:@"E8C14C"];
        
    }
    if(num == 35){
        
        _truckColor = [self colorWithHexString:@"E784FF"];
        _bagColor = [self colorWithHexString:@"A379E8"];
        _obstacleColor = [self colorWithHexString:@"9096FF"];
        _backColor = [self colorWithHexString:@"85B3E8"];
        
    }
    if(num == 36){
        
        _truckColor = [self colorWithHexString:@"FF530D"];
        _bagColor = [self colorWithHexString:@"E82C0C"];
        _obstacleColor = [self colorWithHexString:@"E80C7A"];
        _backColor = [self colorWithHexString:@"FF0DFF"];
        
    }
    if(num == 37){
        
        _truckColor = [self colorWithHexString:@"6C3828"];
        _bagColor = [self colorWithHexString:@"EAB247"];
        _obstacleColor = [self colorWithHexString:@"D3D06C"];
        _backColor = [self colorWithHexString:@"255D55"];
        
    }
    if(num == 38){
        
        _truckColor = [self colorWithHexString:@"A61723"];
        _bagColor = [self colorWithHexString:@"275D8C"];
        _obstacleColor = [self colorWithHexString:@"BFB63B"];
        _backColor = [self colorWithHexString:@"BFB6AA"];
        
    }
    if(num == 39){
        
        _truckColor = [self colorWithHexString:@"5F1BBF"];
        _bagColor = [self colorWithHexString:@"5CD923"];
        _obstacleColor = [self colorWithHexString:@"C4D923"];
        _backColor = [self colorWithHexString:@"F2A71B"];
        
    }
    if(num == 40){
        
        _truckColor = [self colorWithHexString:@"F21E26"];
        _bagColor = [self colorWithHexString:@"1B4059"];
        _obstacleColor = [self colorWithHexString:@"5786A8"];
        _backColor = [self colorWithHexString:@"704823"];
        
    }
    if(num == 41){
        
        _truckColor = [self colorWithHexString:@"F00064"];
        _bagColor = [self colorWithHexString:@"3C1923"];
        _obstacleColor = [self colorWithHexString:@"FF6600"];
        _backColor = [self colorWithHexString:@"401A00"];
        
    }
    if(num == 42){
        
        _truckColor = [self colorWithHexString:@"B34700"];
        _bagColor = [self colorWithHexString:@"FFC219"];
        _obstacleColor = [self colorWithHexString:@"FF6600"];
        _backColor = [self colorWithHexString:@"00A9B3"];
        
    }
    if(num == 43){
        
        _truckColor = [self colorWithHexString:@"E0DCD1"];
        _bagColor = [self colorWithHexString:@"C9C8BD"];
        _obstacleColor = [self colorWithHexString:@"948C81"];
        _backColor = [self colorWithHexString:@"6797EB"];
        
    }
    if(num == 44){
        
        _truckColor = [self colorWithHexString:@"F2CF71"];
        _bagColor = [self colorWithHexString:@"A58536"];
        _obstacleColor = [self colorWithHexString:@"D9B263"];
        _backColor = [self colorWithHexString:@"3F3D3B"];
        
    }
    if(num == 45){
        
        _truckColor = [self colorWithHexString:@"5B8251"];
        _bagColor = [self colorWithHexString:@"303030"];
        _obstacleColor = [self colorWithHexString:@"C45131"];
        _backColor = [self colorWithHexString:@"F16B46"];
        
    }
    if(num == 46){
        
        _truckColor = [self colorWithHexString:@"FF0100"];
        _bagColor = [self colorWithHexString:@"E82CEB"];
        _obstacleColor = [self colorWithHexString:@"E80C7A"];
        _backColor = [self colorWithHexString:@"FF0D58"];
        
    }
    if(num == 47){
        
        _truckColor = [self colorWithHexString:@"313540"];
        _bagColor = [self colorWithHexString:@"FABA2D"];
        _obstacleColor = [self colorWithHexString:@"E9405A"];
        _backColor = [self colorWithHexString:@"4AE9FF"];
        
    }
    if(num == 48){
        
        _truckColor = [self colorWithHexString:@"D7115A"];
        _bagColor = [self colorWithHexString:@"DA177F"];
        _obstacleColor = [self colorWithHexString:@"27BEA2"];
        _backColor = [self colorWithHexString:@"A6D638"];
        
    }
    if(num == 49){
        
        _truckColor = [self colorWithHexString:@"A9F185"];
        _bagColor = [self colorWithHexString:@"FFCC26"];
        _obstacleColor = [self colorWithHexString:@"FF63D9"];
        _backColor = [self colorWithHexString:@"E60B7E"];
        
    }
    if(num == 50){
        
        _truckColor = [self colorWithHexString:@"5D5080"];
        _bagColor = [self colorWithHexString:@"8253FF"];
        _obstacleColor = [self colorWithHexString:@"BA9FFF"];
        _backColor = [self colorWithHexString:@"412980"];
 ...
 //   _obstacleColor = [UIColor whiteColor];
    ...
    for (int j = 0; j < _obstacleArray.count; j++) {
        [[_obstacleArray objectAtIndex:j] setColor:[UIColor whiteColor]];
    }
    ...
    for (UITouch *touch in touches) {
        ...
        */
        ...
        if(_gameHasStarted == TRUE && _gameIsOver == false){
            _xTouchInput = (location.x - 160*_sizeChanger)*1.2 + 160*_sizeChanger;
...
-(void)movePlayer{
    ...
    if(_xTouchInput > self.size.width){
        _xTouchInput = self.size.width;
    }
...
//    lastHighScore = 0;
    ...
    SKSpriteNode *testShape = (SKSpriteNode *)[testNode_ForLocalScore childNodeWithName:@"scoreboxNode"];
    ...
    SKSpriteNode *scoreBox = [SKSpriteNode spriteNodeWithImageNamed:@"scoreBox.png"];
    scoreBox.size = CGSizeMake(50*_sizeChanger, 50*_sizeChanger);
 ...
 //   UIBezierPath *scoreBoxPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, 40*_sizeChanger, 40*_sizeChanger) cornerRadius:cornerSize];
 ...
 //   CGPathRelease(scoreBoxPath);
  ...
  //  scoreBox.position = CGPointMake(self.size.width/2 - scoreBox.frame.size.width/2, self.size.height - scoreBox.frame.size.height/2 - 60*_sizeChanger + cornerSize*2);
    ...
    scoreBox.position = CGPointMake(self.size.width/2 + scoreBox.frame.size.width*0/2, self.size.height + scoreBox.frame.size.height/2 - 60*_sizeChanger + cornerSize*0*2);
   ...
   // backLabel.position = CGPointMake(scoreBox.frame.size.width/2 , scoreBox.frame.size.height/2 - backLabel.frame.size.height/2);
    ...
    backLabel.position = CGPointMake(0, scoreBox.frame.size.height*0/2 - backLabel.frame.size.height/2);
    ...
    localPlayer.authenticateHandler = ^(UIViewController *viewController, NSError *error){
        if (viewController != nil) {
            ...
            if ([GKLocalPlayer localPlayer].authenticated) {
                ...
                currentLeaderBoard = @"rekt_score_ID";
...
-(void)facebook{
    ...
    // https://itunes.apple.com/us/app/rekt/id889559209?ls=1&mt=8
    FBLinkShareParams *params = [[FBLinkShareParams alloc] init];
    params.picture = [NSURL URLWithString:@"http://www.imgur.com/VHrXfov.png"];
  //  params.link = [NSURL URLWithString:@"https://itunes.apple.com/us/app/rekt/id889559209?ls=1&mt=8"];
    ...
    NSURL *appUrl = [NSURL URLWithString:@"https://itunes.apple.com/us/app/rekt/id889559209?ls=1&mt=8"];
 //   NSURL *appUrl = [NSURL URLWithString:@"https://www.google.com"];

   NSURL *picUrl = [NSURL URLWithString:@"http://www.imgur.com/VHrXfov.png"];
   ...
   if ([FBDialogs canPresentShareDialogWithParams:params]) {
    ...
    /*
        ...
        *//*
        ...
        NSString *descriptString = [NSString stringWithFormat:@"Get REKT! I got %d. From @TheRowdyPanda #REKT", _score];
       NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                       @"Rekt!", @"name",
                                       @"From Rowdy Panda Games", @"caption",
                                       descriptString, @"description",
                                       @"https://itunes.apple.com/us/app/rekt/id889559209?ls=1&mt=8", @"link",
                                       @"http://imgur.com/VHrXfov.png", @"picture",
                                       nil];
...
-(void)twitter{
   ...
   // int score = 10;
    NSString *initialTextString = [NSString stringWithFormat:@"Get REKT! I got %d. From @TheRowdyPanda #REKT", _score];
    [tweetSheet setInitialText:initialTextString];
    
    //  Add an URL to the Tweet.  You can add multiple URLs.
    if (![tweetSheet addURL:[NSURL URLWithString:@"https://itunes.apple.com/us/app/rekt/id889559209?ls=1&mt=8"]]){
        NSLog(@"Unable to add the URL!");
    }
    
    //  Adds an image to the Tweet.  For demo purposes, assume we have an
    //  image named 'larry.png' that we wish to attach
    //http://imgur.com/VHrXfov.png
    //NSURL *imageURL = [NSURL URLWithString:@"http://example.com/demo.jpg"];
 //   NSData *imageData = [NSData dataWithContentsOfURL:imageURL];
 //   UIImage *image = [UIImage imageWithData:imageData];
    //
    //
    NSURL *imageURL = [NSURL URLWithString:@"http://imgur.com/VHrXfov.png"];
    //   NSData *imageData = [NSData dataWithContentsOfURL:imageURL];
       UIImage *image = [UIImage imageNamed:@"twitPic.png"];
...
- (void)updateWithTimeSinceLastUpdate:(CFTimeInterval)timeSinceLast {
    ...
    self.timeSinceLastBag += timeSinceLast;
    self.timeSinceLastCar += timeSinceLast;
    ...
    double timeAddCarInterval = 0.25 + num2/60.0;

    if(round(_score/(_pointMultiplyer.integerValue)) <= 250){
        timeAddCarInterval = timeAddCarInterval - arc4random()%10/100.0;
    }
    timeAddCarInterval = 0.20;
 
    if(_timeSinceLastCar >= timeAddCarInterval){
        [self addCar];
        _timeSinceLastCar = 0.0;
    }
    ...
    double timeAddBagInterval = 0.2 + num/300.0 + arc4random()%10/100.0;

    
    if(_timeSinceLastBag >= timeAddBagInterval){
        _timeSinceLastBag = 0.0;
        [self addLawns];
    }
...
-(SKEmitterNode *)addEndgame{
    ...
    _endGameEmitter.particleTexture = [SKTexture textureWithImage:[UIImage imageNamed:@"whiteBox.png"]];
    _endGameEmitter.particleScale = 0.33*_sizeChanger;
...
-(void)increasePointMultipler{
 ...
 //   [self gameOver];
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:@"lastTimeSharedForIncrease"];//set's saved date as today's date.
